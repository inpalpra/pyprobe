...
- [x] Task: Streamline `docker/Dockerfile` 24219e6
    - [x] Reference the updated base image.
    - [x] Remove `pip install` or `uv sync` blocks that are now redundant.
    - [x] Update to only copy source and install the project itself (`uv sync --frozen`).
- [x] Task: Refactor `scripts/test_artifact.sh` 24219e6
    - [x] Remove all blocks that perform `pip install` of test dependencies (pytest-qt, scipy, etc.) at runtime inside the container.
